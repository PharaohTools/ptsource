...
function updatePage() {
    console.log("running update page js method");
        ...
        url = "/index.php?control=PipeRunner&action=findrunning&repository="+item+"&output-format=JSON";
        $.ajax({
            url: url,
            success: function(data) {
                setRunningRepositoryList(data) ;
                window.updateRunning = false ; } ,
...
var row;
function setRunningRepositoryList(data) {
    data = JSON.parse(data);
    console.log(data);
    if (data.length == 0) {
        $('.runningRepositoryRow' +" > td ").animate({ opacity: 100 });
        $('.runningRepositoryRow' +" > th ").animate({ opacity: 100 });
        $('.buildRow').removeClass("runningRepositoryRow");
        ht = "<p>No builds currently being executed...</p>" ;
        $('#runningRepositories').html(ht); }
    else {
        ht = "" ;
        for (index = 0; index < data.length; index++) {
            ...
            $('#blRow_'+data[index].item).addClass("runningRepositoryRow");
            $('.runningRepositoryRow' +" > td ").animate({ opacity: 0 });
            $('.runningRepositoryRow' +" > th ").animate({ opacity: 0 });
...
//            ht += '  <h5><strong>Repository:</strong> '+data[index].item+'</h5>' ;
            ht += '  <h5><a href="index.php?control=PipeRunner&action=show&item=' ;
            ht += data[index].item+'&run-id='+data[index].runid+'">' ;
            ht += '  <strong>Repository start at:</strong> '+data[index].starttime+'</a></h5>' ;
