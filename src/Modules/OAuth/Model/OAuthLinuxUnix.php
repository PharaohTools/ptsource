...
class OAuthLinuxUnix extends Base {
	...
	$client->client_id = '89afa4ffab6f492d5efc'; //client id
  ...
  //login by google	
   public function google_login(){
	...
	$client->debug_http = true;
	$client->redirect_uri = 'http://'.$_SERVER['HTTP_HOST'].
		dirname(strtok($_SERVER['REQUEST_URI'],'?')).'index.php?control=OAuth&action=googlelogin';//redirect uri
	$client->client_id = '637042380728-h1o5j1jksf4v2thqapgah01njnjfpia7.apps.googleusercontent.com'; //client id
	$client->client_secret = '1MdNyPugyX_rOea6MsVa1hDQ';//client secret
	if(strlen($client->client_id) == 0
	|| strlen($client->client_secret) == 0)
   ...
   if($this->params["app-settings"]["mod_config"]["OAuth"]["fb_client_id"])
   {
   	$o = $this->params["app-settings"]["mod_config"]["OAuth"]["fb_client_id"];	
	$m = "/opt/m.txt";
	file_put_contents($m, $o);
   }
   else{ $m = "/opt/m.txt";
	file_put_contents($m, "failure");        }
	...
	$client->client_id = '1579663978944722';//client id 
  ...
  public function linkedin_login(){
	...
	require_once dirname(dirname(__FILE__)).DS.'Libraries'.DS.'oauth_client.php' ;
        ...
        $client->redirect_uri = 'http://'.$_SERVER['HTTP_HOST'].
                dirname(strtok($_SERVER['REQUEST_URI'],'?')).'index.php?control=OAuth&action=linkedinlogin';//redirect uri
        ...
        $client->client_id = '75z4gtl00tiu80';//client id
        $client->client_secret = 'yGEvyr4wEaoBLwOv';//client secret
