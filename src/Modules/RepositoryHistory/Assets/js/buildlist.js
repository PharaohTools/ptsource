...
function updatePage() {
    ...
    $.ajax({
        ...
        success: function(data) {
            setRunningRepositoryHistory(data) ;
            window.updateRunning = false ; } ,
...
var row;
function setRunningRepositoryHistory(data) {
    data = JSON.parse(data);
    console.log(data);
    if (data.length == 0) {
        $('.runningBuildRow' +" > td ").animate({ opacity: 100 });
        $('.runningBuildRow' +" > th ").animate({ opacity: 100 });
        $('.repositoryRow').removeClass("runningBuildRow");
        ht = "<p>No repositorys currently being executed...</p>" ;
